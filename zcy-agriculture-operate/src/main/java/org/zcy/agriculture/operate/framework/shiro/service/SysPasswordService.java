...
import org.zcy.agriculture.operate.common.exception.user.UserPasswordRetryLimitExceedException;
...
public class SysPasswordService
{
    ...
    public void validate(SysUser user, String password)
    {
        ...
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(loginName, ConstantsOp.LOGIN_FAIL, MessageUtils.message("user.password.retry.limit.exceed", maxRetryCount)));
            throw new UserPasswordRetryLimitExceedException(Integer.valueOf(maxRetryCount).intValue());
        }

        if (!matches(user, password))
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(loginName, ConstantsOp.LOGIN_FAIL, MessageUtils.message("user.password.retry.limit.count", retryCount)));
            loginRecordCache.put(loginName, retryCount);
