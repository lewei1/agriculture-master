package org.zcy.agriculture.operate.framework.shiro.web.filter;
...
public class LogoutFilter extends org.apache.shiro.web.filter.authc.LogoutFilter
{
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception
    {
        try
        {
            ...
            try
            {
                ...
                if (StringUtilsOp.isNotNull(user))
                {
                    String loginName = user.getLoginName();
                    // 记录用户退出日志
                    AsyncManager.me().execute(AsyncFactory.recordLogininfor(loginName, ConstantsOp.LOGOUT, MessageUtils.message("user.logout.success")));
                }
                // 退出登录
                subject.logout();
            }
    ...
    protected String getRedirectUrl(ServletRequest request, ServletResponse response, Subject subject)
    {
        ...
        if (StringUtilsOp.isNotEmpty(url))
        {
            return url;
        }
